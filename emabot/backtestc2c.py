...
def _backtest(df, emaA: int = 1, emaB: int = 2, progress_bar: tqdm = None, debug: bool = False):
    ...
    for (timestamp, row) in df.iterrows():
        ...
        if not bought and emaA > emaB:
            ...
            if debug:
                print('{} BOUGHT: price={:,.8f} A={:,.8f} B={:,.8f} '
                    'size={:,.8f} fee={:,.8f} wallet={:,.8f}'.format(
            ...
            if debug:
                print('{} SOLD : price={:,.8f} A={:,.8f} B={:,.8f} '
                    'profit={:,.8f} percent={:,.8f} wallet={:,.8f}'.format(
                        timestamp, cur_price, emaA,
...
def main() -> None:
    ...
    for timestamp in stats.per_day['fee'].keys():
        day_results.append((
            timestamp,
            #'{:,.8f}'.format(sum(stats.per_day['fee'][timestamp])),
            #'{:,.8f}'.format(np.mean(stats.per_day['fee'][timestamp])),
            #'{:,.8f}'.format(np.median(stats.per_day['fee'][timestamp])),
            ...
            #'{:,.8f}'.format(sum(stats.per_day['net_profit'][timestamp])),
            #'{:,.8f}'.format(np.mean(stats.per_day['net_profit'][timestamp])),
            #'{:,.8f}'.format(np.median(stats.per_day['net_profit'][timestamp])),
        ))
